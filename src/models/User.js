...
export default class User {
   ...
   *          phone } */
	...
	static async addMovie (username, movieId, favorite = false, rating = null, watched = false, watchlist = false) {
   ...
   */
	...
	static async updateUserMovie (username, movieId, data) {
		try {
			...
			if (data.favorite !== undefined) return await User.addOrRemoveFromFavorites(data.favorite, username, movieId);
			if (data.watched) return await User.markAsWatched(username, movieId);
			if (data.watched === false) return await User.markAsUnwatched(username, movieId);
